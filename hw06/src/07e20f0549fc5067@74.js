...
Plot.plot({
  ...
  color: {
    ...
    range: ['#7DB9DE', '#2B5F75'],  // 你可以根據需要更改顏色
    legend: true
  },
  marks: [
...
function _chart_1(data,selectedSeries1,d3)
{
  // 定義邊界大小，以及圖形的寬度和高度
  const margin = {top: 20, right: 30, bottom: 30, left: 40};
  const width = 500 - margin.left - margin.right;
...
function _chart_2(data,selectedSeries1,d3)
{
  // 定義邊界大小，以及圖形的寬度和高度
  const margin = {top: 20, right: 30, bottom: 30, left: 40};
  const width = 500 - margin.left - margin.right;
...
// 创建阴影滤镜
  //svg.append("defs").append("filter")
    // .attr("id", "stackedBarShadow")
    // .attr("height", "130%")
    // .append("feGaussianBlur")
    // .attr("in", "SourceAlpha")
    // .attr("stdDeviation", 10)  // 可以调整这个值
    // .attr("result", "blur")
    // .append("feOffset")
    // .attr("in", "blur")
    // .attr("dx", 4)  // 调整这里的值
  ...
  series.forEach((serie) => {
      ...
      bars.enter().append("rect")
          ...
          .attr("y", d => yScale(d[1]))
          .attr("height", d => yScale(d[0]) - yScale(d[1]))
          .attr("filter", "url(#stackedBarShadow)")
          .on("mouseover", function(d) {
              d3.select(this).attr("fill", "#86C166");     
            })
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["artistPublic.csv", {url: new URL("./files/artistPublic.csv", import.meta.url), mimeType: "text/csv", toString}],
    ["artistVer.csv", {url: new URL("./files/artistVer.csv", import.meta.url), mimeType: "text/csv", toString}]
  ]);
  ...
  main.variable(observer("chart_1")).define("chart_1", ["data","selectedSeries1","d3"], _chart_1);
  main.variable(observer("chart_2")).define("chart_2", ["data","selectedSeries1","d3"], _chart_2);
  return main;
}
